...
- (void)layoutSubviews {
    if (shouldUpdateSubviews == YES) {
        ...
        UIImageView *hour_image = [[UIImageView alloc] initWithImage:[UIImage imageNamed: ([model isEqual: @"iPad"]) ? @"hour_hand_ipad" : @"hour_hand"]];
        [hour_image setContentMode:UIViewContentModeScaleAspectFit];
        NSLog(@" hour fraem img= %@",NSStringFromCGRect(hour_image.frame));
        hour_image.center = CGPointMake(self.hourHand.center.x, self.hourHand.center.y-(hour_image.frame.size.width+8));
        [self.hourHand addSubview: hour_image];
        ...
        UIImageView *minute_image = [[UIImageView alloc] initWithImage:[UIImage imageNamed:([model isEqual: @"iPad"]) ? @"minute_hand_ipad" : @"minute_hand"]];

        NSLog(@" minute fraem img= %@",NSStringFromCGRect(minute_image.frame));
        [minute_image setContentMode:UIViewContentModeScaleAspectFit];
        
        if ([model isEqual: @"iPad"]) {
            minute_image.center = CGPointMake(self.minuteHand.center.x, self.minuteHand.center.y-(minute_image.frame.size.width+12.7));
        } else {
            minute_image.center = CGPointMake(self.minuteHand.center.x, self.minuteHand.center.y-(minute_image.frame.size.width+6.7));
        }
       [self.minuteHand addSubview: minute_image];
        
        //self.minuteHand. = self.minuteHandAlpha;
...
-(void) allowFingerToMoveClock:(BOOL)flag {
    
    if (flag) {
        [_panView removeFromSuperview];
        _panView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        _panView.backgroundColor = [UIColor clearColor];
        [self.minuteHand.viewForLastBaselineLayout addSubview:_panView];
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer {
    ...
    if ((vel.x > 0) == false)
    {
        NSLog(@"user dragged towards the left");
//        if ([self.delegate respondsToSelector:@selector(userMovingHandAntiClockwise)]) {
//            [self.delegate userMovingHandAntiClockwise];
//        }
//        return;
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        if ([self.delegate respondsToSelector:@selector(userPutsOfHisFingureOutOfClock)]) {
            [self.delegate userPutsOfHisFingureOutOfClock];
        }
    }
}
